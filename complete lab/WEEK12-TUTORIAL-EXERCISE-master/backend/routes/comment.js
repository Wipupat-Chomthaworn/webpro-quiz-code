...
const commentOwner = async (req, res, next) => {
    const [[comments]] = await pool.query('SELECT * FROM comments WHERE id=?', [req.params.commentId])
    if (req.user.role === 'admin') {
        return next()
    }
    if (comments.comment_by_id !== req.user.id) {
...
// Add Like comment
router.put('/comments/addlike/:commentId', isLoggedIn, async function (req, res, next) {
    const conn = await pool.getConnection()
    await conn.beginTransaction();
